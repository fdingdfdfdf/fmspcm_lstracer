...
contains
  ...
  subroutine doppler(delta_nu,gammD,fD)
    ...
    real, intent(in)  :: delta_nu ! (nu-nu0) [cm^-1]
    real, intent(in)  :: gammD    ! Doppler linewidth (not HWHM!) [cm^-1]
    real, intent(out) :: fD       ! Doppler line profile [1/cm^-1]
  ...
  subroutine lorentz(delta_nu,gammL,fL)
    ...
    real, intent(in)  :: delta_nu ! (nu-nu0) [cm^-1]
    real, intent(in)  :: gammL    ! Lorentz linewidth [cm^-1]
    real, intent(out) :: fL       ! Lorentz line profile [1/cm^-1]
  ...
  subroutine voigt(gammL, gammD, delta_nu, fV)
    ...
    real, intent(in)  :: gammD    ! Doppler linewidth (not HWHM!) [cm^-1]
    real, intent(in)  :: delta_nu ! (nu-nu0) [cm^-1]
    real, intent(out) :: fV       ! Voigt profile [1/cm^-1]
