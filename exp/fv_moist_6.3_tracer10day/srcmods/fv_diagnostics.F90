module fv_diagnostics_mod
 ...
 use constants_mod,    only: grav, rdgas, pi, radius, kappa, nS
...
!   Approach will need modification if we wish to write values on other than A grid.
    ...
    allocate(wavenumber(nS))
    do i=1,nS
       wavenumber(i) = i
    end do
    id_wn = diag_axis_init('nS', wavenumber, 'cm^-1', 'z', &
         'wavenumber', direction=-1, set_name="dynamics")
    axes(5) = id_wn
...
!-------------------
       id_ts = register_diag_field ( trim(field), 'ts', axes(1:2), Time,           &
            'surface temperature', 'K', missing_value=missing_value )
!-------------------
! Surface pressure
!-------------------
