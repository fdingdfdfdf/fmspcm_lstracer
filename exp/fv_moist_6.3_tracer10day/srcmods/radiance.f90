...
! modif omp: winter/summer hemisphere
  real    :: del_sw          = 0.
  real    :: kappa_gray = 0.1 !.5d-4 !0.01 !5.0d-3 
...
!  real :: deltanu_trunc = 25.0d0      ! truncation wavenumber [cm^-1]
  ...
  logical :: stellar_blackbody = .true. !.false.  ! use a blackbody curve for the stellar spectrum?
  ...
  logical :: gray_debug   = .true. !.false. ! uniform opacity vs. wavenumber
  !------- namelist parameters --------
  real :: nu_sw1 = 1.0                               ! longwave starting wavenumber [cm^-1]
  real :: nu_sw2 = 50000.0                              ! longwave finishing wavenumber [cm^-1]
  real :: Asurf  = 0.25                             ! surface albedo []
...
namelist/radiance_nml/  solar_constant, nGas, nAng, nTem,  mCO2, & 
        tidal_lock, stellar_blackbody, gray_debug
!==================================================================================
!-------------------- diagnostics fields -------------------------------
...
!f_i(:,:,:,2) = mCO2 !400.e-6, molar mixing ratio relative to dominate dry component
...
!zonal-symmeric
    ...
    Fstel0(:,:) = solar_constant * cosa0(:,:) /pi
endif
...
!       do iGas = 1,nGas
          do iLay = 1, nLay
             ...
             dtau_sw(iLay,:,:) = dtau_sw(iLay,:,:) + &
                   1.0d-4*sigma_sw_total_dry(iLay,:,:)*dp(1,1,iLay)/(grav*mu_avg(1,1,iLay)*mpr)
