...
module init_hydro_mod
      ...
      use constants_mod, only: grav, rdgas, ps0, kappa
...
! Convert geopotential to Temperature
      do k=1,km
         do i=is,ie
            ...
            !df, pure dry adiabat
            pt(i,j,k) = t0 * ((ph(i,k+1)+ph(i,k))/2./ps(i,j))**(2./7)
         enddo
      enddo
   enddo    ! j-loop
...
!df
  if ( gid==masterproc ) write(*,*) 'DF Initializing Profile'
      p0 = ps0     !initial surface pressure    
      t0 = 280.    !initial surface t
      pt = t0      !initial air t
! gztop when zs==0
      gztop = rdgas*t0*log(p0/ak(1))
     do j=js,je
     ...
     enddo
     psm = g_sum(ps(is:ie,js:je), is, ie, js, je, ng, area, 1, .true.)
     dps = ps0 - psm !drym - psm
     if(gid==masterproc) write(6,*) 'DF Initializing:  Computed mean ps=', psm
     if(gid==masterproc) write(6,*) '            Correction delta-ps=', dps
!           Add correction to surface pressure to yield desired
!                globally-integrated atmospheric mass  (drym)
     do j=js,je
...
!      ps(is:ie, js:je) = p0
      do k=1,km
         delp(is:ie,js:je,k) = ak(k+1)-ak(k) + ps(is:ie,js:je)*(bk(k+1)-bk(k))
         !df, pure dry adiabat
         pt(is:ie,js:je,k) = t0 *  &
                ( ( (ak(k+1)+ak(k))/ps(is:ie,js:je)+bk(k+1)+bk(k))/2.) &
                **kappa !(2./7)
      enddo
